...
export function js<T extends unknown[]>(
  ...
  return html`<script>
    (${fn.toString()}).apply(this, ${JSON.stringify(args)});
  </script>`;
}
...
export function component<El extends HTMLElement, Props extends unknown[]>(
  ...
  return (...props: Props) => {
    ...
    if (script) {
      ...
      if (map.has(script)) {
        id = map.get(script);
        returned += `<script>window["${id}"].apply(document.currentScript.previousElementSibling)</script>`;
      } else {
        id = randomBytes(4).toString("hex");
        map.set(script, id);
        returned += `<script>(window["${id}"]=${script.toString()}).apply(document.currentScript.previousElementSibling)</script>`;
      }
    }
